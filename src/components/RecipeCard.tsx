  ...
  return (
    <article className="parchment-bg paper-edge animate-slide-in relative mx-auto flex min-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-lg p-5 sm:min-h-[80vh] sm:p-6 md:p-10 lg:min-h-[75vh]">
      ...
      <div className="relative z-10 flex-1 pr-4 sm:pr-8 md:pr-16">
        ...
        {/* External Links */}
        {recipe.externalLinks && recipe.externalLinks.length > 0 && (
          <section className="mt-4 sm:mt-6">
            <div className="flex flex-wrap gap-2">
              {recipe.externalLinks.map((link, index) => (
                <a
                  key={index}
                  href={link.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 rounded-md bg-warm-brown/10 px-3 py-1.5 text-sm text-warm-brown transition-colors hover:bg-warm-brown/20"
                >
                  {link.text}
                  <svg
